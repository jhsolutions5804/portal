...
| 파일 | 위치 |
|------|------|
| hr/index.html   | backup/v2.6.1/hr/index.html |
| edoc/index.html | backup/v2.6.1/edoc/index.html |

### 주요 변경 내용
- **초과근로 상신 시 사유 입력 필드 추가**
  - `edoc/index.html`: "초과근로 작성" 폼에 사유 텍스트영역 추가 → `edoc_overtime` 문서에 `reason` 저장, 상세보기 표시, 결재 승인 시 인사 `overtime` 컬렉션 연동에도 `reason` 함께 전달
  - `hr/index.html`: 초과근로 탭(PC/모바일) 입력·수정 폼에 사유(선택) 필드 추가, 각 상세/조회 테이블에 사유 컬럼 표시
  - 사유는 선택 입력(필수 아님), 기존 데이터는 자동으로 "-" 표시 (마이그레이션 불필요)
- 메인 `index.html` 버전주석: build 20260721 · edoc 3.2r1

...
**공급가액 직접 수정** (r8, 2026-08-18 변경 — 이전엔 합계금액 직접수정)
- 마지막 품목 줄이 차액 흡수
- 부가세 = 공급가액 × 10%, 합계금액 = 공급가액 + 부가세 (모두 자동계산, 읽기전용)
- 기존엔 합계금액(부가세 포함)을 직접 입력받아 공급가 = 합계/1.1로 역산했는데, 나눗셈 반올림 오차가 발생할 수 있어 공급가액을 직접 입력받는 방식으로 변경. `onSupplyEdit(v)`가 담당 (구 `onTotalEdit` 대체)

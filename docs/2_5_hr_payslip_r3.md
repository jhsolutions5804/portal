...
## UI — 금액 입력 포맷 (r6, 2026-07-23)

지급내역(기본급 등)·공제내역(4대보험 등)·상여금·특별상여·기타수당·기숙사공제·시급(일용직/연봉직) 입력창을 `type="text"` + 실시간 콤마 포맷(`fmtMoney(el)`)으로 전환(1000단위마다 `,`). 파싱은 `numClean(v)` 헬퍼로 통일(콤마 포함 문자열 안전 처리). 상세: `7_3_log_hr_r10b.md`


---

## 포괄임금제 폐지 및 실근무기록 기반 자동계산 전면 개편 (r7, 2026-08-29)

### 배경
기존 급여 구조는 연봉계약서에 고정된 "고정연장수당"/"고정야간수당"을 매달 그대로 지급하는 포괄임금제 방식이었음. 이를 폐지하고, 매달 실제 근무기록(출퇴근 기록·유급휴가·야간·휴일근무)을 기준으로 급여명세서 작성 시점에 자동 계산하는 방식으로 전면 개편.

### 새 계산 로직

| 항목 | 계산식 | 비고 |
|------|--------|------|
| **기본근로수당** | 시급×209h 산정, 단 그 달 캘린더 기준 소정근로시간(`monthlyStandardHours`, 매달 변동) 대비 실근무시간(출퇴근+유급휴가) 부족 시 부족분만큼 차감 | 209h는 "금액 크기" 산정용, 초과/부족 "판정 기준"은 캘린더 기준 소정근로시간 |
| **연장근로수당** | (실근무시간 − 그 달 소정근로시간) × 시급 × 1.5 | 초과분이 있을 때만 |
| **야간근로수당** | 22:00~06:00 실근무시간(`hrCalcNightHours`) × 시급 × 0.5 | 출퇴근기록 checkIn/checkOut 기준 |
| **휴일근로수당** | 토·일·공휴일 근무시간 중 8h 이내×1.5, 8h 초과분×2.0 (`hrCalcHolidayPay`) | 평일 근무시간과 분리 집계 |
| **주휴수당** | 그 주 소정근로일 개근(출퇴근기록 또는 승인된 휴가로 커버) 시 1일(8h)분 지급, 결근한 주는 미지급 (`hrCalcWeeklyHolidayPay`) | 출퇴근기록이 그 달에 전혀 없으면 기존 고정계산(시급×8×4.345)으로 폴백 |

`fixedOt`/`fixedNight` Firestore 필드명은 과거 저장 데이터와의 호환을 위해 그대로 유지하되, 값의 의미만 "고정수당→실근무 기반 연장/야간근로수당"으로 변경됨.

### 근태 데이터 연동 (`psFetchAttendanceData`)
근로자 선택·기준월 변경 시 `worker_attendance_log`(출퇴근기록 누계·야간시간·일자별 원시데이터), `edoc_leave`(유급휴가) 조회 → `ps.attHours`/`ps.leaveHours`/`ps.nightHours`/`ps.attDates`/`ps.leaveDocsRaw`/`ps.holidayRecords`에 캐싱 → `psCalcAll()`이 동기적으로 읽어 계산.

⚠️ **중요 버그(수정 완료)**: PC 화면(`psPcWorkerSelect`)에 이 근태 데이터 연동 로직이 누락되어 있어, 실제 근무와 무관하게 "0시간 근무"로 오인해 기본급이 대폭 삭감되고 주휴수당이 옛 고정값으로 나오던 문제가 있었음. 모바일(`psWorkerSelect`)에만 있던 로직을 PC에도 동일하게 추가해 해결.

### 연봉계약서 화면 변경
고정연장/고정야간수당 항목 완전 제거. 대신 "포괄임금제 미적용 — 연장·야간근로수당은 매달 실제 근무기록 기준 자동계산됩니다" 안내 문구 추가. 사무직/현장직 구분(`empType`)에 따른 별도 산식 분기도 제거(양쪽 다 기본근로수당+주휴수당만 남아 동일 산식).

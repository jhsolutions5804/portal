# 1.4. 기획 — 정산

> Firestore 컬렉션: `gihoek_settlements`
> 최초 작성: 2026-07-01 · 최종 수정: 2026-08-18(r9) · 작성: 춘식이(Claude)

---

## Firestore 스키마

```js
gihoek_settlements/{id}:
{
  docType:   'invoice' | 'payment',   // 대금청구서(매출) | 지급예정서(하청 지급)
  no:        string,                  // 문서 번호
  pjtId:     string,                  // 대표(주) 프로젝트 (목록 필터·상단 표시 기준)
  pjtIds:    [string],                // r8 신규 — 공수연동 시 합산 대상 프로젝트 전체 목록 (1개면 pjtId와 동일)
  mandayRateOverride: {pjtId: rate},  // r9 신규 — 공수연동 다중 프로젝트 시 프로젝트별 개별 단가 (없으면 공통 단가 사용)
  baseMonth: string,                  // 기준월 'YYYY-MM'
  status:    'active' | 'done' | 'void',
  issueDate: string,
  payDate:   string,                  // 지급요청일/지급예정일
  method:    'progress' | 'qty' | 'manday' | 'manual',
  sender:    {company, ceo, bizno, contact},   // 발신(공급자)
  recipient: {company, ceo, bizno, contact},   // 수신(원청/하청)
  lines:     [{name, spec, amount}],
  supply:    number,   // 공급가
  vat:       number,   // 부가세
  total:     number,   // 합계
  doneAt:    serverTimestamp,
  createdAt: serverTimestamp
}
```

---

## 문서 유형

| docType | 명칭 | 의미 | 완결 처리 |
|---------|------|------|----------|
| `invoice` | 대금청구서 | 원청에 청구(매출) | `doneSettle` → 회계 매출 인식 |
| `payment` | 지급예정서 | 하청에 지급 | `doneSettlePayment` → 회계 직접비(용역비) 반영 |

---

## 상태 정의

| status | 의미 | 집계 |
|--------|------|------|
| `active` | 유효 | ✅ |
| `done` | 완결(청구는 수금, 지급은 송금 완료) | ✅ |
| `void` | 폐기 | ❌ |

`setActive(s)` = 폐기 아님

---

## 지급예정서 → 용역비 자동연동 (N10-1)

지급예정서를 "✓ 지급완료"(`doneSettlePayment`) 처리하면 회계 비용장부의 직접비(용역비)로 자동 반영된다.

**완결 버튼 노출 (정산 상세):**
- `invoice` + active + status≠done → "✓ 완결" (`doneSettle`)
- `payment` + active + status≠done → "✓ 지급완료" (`doneSettlePayment`)
- status===done → "완결 취소" (`undoneSettle`, docType별 메시지 분기)

**자동 반영 로직** (회계 `paymentAsExpense()`):
```
완결된 payment (status==='done' && baseMonth===현재회계월 && setActive)
  → { cat:'용역비'(직접비), vendor:recipient.company, pjt:pjtId,
      supply/vat/total: 정산서 값 그대로, fromSettlement:true }
```
- 비용장부 상세내역에 "🔗 정산 연동" 태그 표시
- 자동 항목 클릭 → `openSettleFromLedger` → 정산 화면으로 이동(수정 불가)
- 지급완료 취소 시 비용장부에서 자동 제외

> ⚠️ 지급예정서는 "지급완료" 처리 전까지는 손익에 반영되지 않는다. (실제 송금 확인 후 반영)

---

## 지급예정서 회계 계상월 — 1.4.x

지급예정서(payment)가 회계 장부에 계상되는 월(`acctMonth`). 급여와 동일 규칙.

- 기준: **지급예정일(payDate)** 의 월. 지급예정일 비면 `defaultPayDate(기준월)` = 익월 말일(주말→금요일 보정) 자동
- `acctMonthOf(s)`: 저장된 `acctMonth` 우선 → 없으면 payDate→issueDate→baseMonth 폴백(하위호환)
- 발행 시 `acctMonth` 저장. 상세보기에서 표시 + 수정 버튼(`editAcctMonth`, YYYY-MM prompt)
- 회계 반영(`paymentAsExpense`)은 `acctMonth` 기준으로 해당 월 집계


---

## UI — 금액 입력 포맷 (r2, 2026-07-23)

기술인력 단가, 직접입력 정산 품목(단가·금액) 입력창에 실시간 콤마 포맷(`fmtMoney(el)`) 적용(1000단위마다 `,`). 상세: `7_2_log_gihoek_r5.md`

---

## 정산 방식 — 공정 연동(progress)의 구역(zone) 자동인식 — 1.4.x (r3, 2026-07-30)

`method:'progress'`(공정 연동)로 정산서를 작성할 때, 견적 항목을 체크하면 `zoneGuess(title)`가 항목명에 포함된 키워드로 구역(zone)을 자동 판정한다. (예: "3F FIZ", "복합3동", "X-OB" 등 — `PROGRESS_PROFILES[프로젝트].zones`에 정의)

- 판정 방식: `zones` 배열을 순서대로 검사해 **키워드가 처음 매치되는 zone**으로 확정 (first-match)
- **주의(2026-07-30 버그 수정)**: 견적 항목명에 건물명(예: "복합4동")과 설비타입(예: "3F FIZ")이 함께 들어가는 경우가 있음 (예: "복합4동 3F FIZ FCU 14대 설치"). 이때 `zones` 배열에서 설비타입 zone("3F FIZ")이 건물명 zone("복합4동")보다 먼저 나오면, 실제로는 복합4동(14대)인 항목이 3F FIZ(39대)로 잘못 배정되어 반입/설치 진행률이 엉뚱한 총 수량(39) 기준으로 표시되는 문제가 있었다.
- **수정**: `PROGRESS_PROFILES.p4ph2.zones` 배열 순서를 건물명 zone(복합3동·복합4동)이 설비타입 zone(3F FIZ 등)보다 **먼저 오도록** 재배치. `zoneGuess` 로직 자체는 유지(first-match).
- ⚠️ 자동 인식은 **신규 체크 시점**에만 적용된다. 기존에 이미 잘못 배정되어 저장된 정산서(`estZone`)는 자동 반영되지 않으므로, 정산서 상세 화면의 구역 선택 드롭다운(`setZone`)으로 수동 재선택 필요.
- 향후 zone을 추가할 때는 항목명에 건물명/구역명이 설비타입명과 함께 쓰일 가능성을 고려해, **더 구체적인(건물·구역 고유명) 키워드를 배열 앞쪽에** 배치할 것.

---

## 공급가액·부가세 수동 수정 — 1.4.x (r4, 2026-07-30)

정산서 작성 화면 하단 합계 박스(공급가액/부가세)를 직접 입력으로 수정할 수 있다.

- 기본값은 정산 방식(공정/수량/공수/직접입력)에 따라 자동 계산된 값(`setSupply()`)이며, 부가세는 공급가액의 10%
- 공급가액 또는 부가세 입력창에 직접 값을 입력하면 `settleDraft.supplyOverride` / `vatOverride`에 저장되어 그 값이 우선 적용됨(`effSupply()`/`effVat()`)
- 수동 수정된 상태에서는 "✎ 수동 수정됨" 안내와 함께 자동계산값을 같이 표시, "자동계산으로 되돌리기" 링크로 오버라이드 해제 가능
- 발행(`publishSettle`) 시 오버라이드된 값 그대로 `gihoek_settlements`에 저장됨 (`supply`, `vat`, `total`)
- ⚠️ 오버라이드는 견적/공수 등 원본 항목을 수정해도 자동으로 재계산되지 않고 유지됨 — 원본 데이터를 바꾼 뒤에는 필요 시 "자동계산으로 되돌리기"로 재확인할 것

---

## 공수 연동(manday) 프로젝트별 단가 개별 수정 — 1.4.x (r9, 2026-08-18)

**요청**: r8에서 추가한 다중 프로젝트 합산 시, 단가가 전체 프로젝트에 공통으로 적용되는 하나뿐이었는데 프로젝트별로 단가를 다르게 지정할 수 있게 해달라는 요청 (예: FAB 팀단가와 SUP 팀단가가 다른 경우).

**구현**:
- `settleDraft.mandayRateOverride`(객체, pjtId→rate) 신규
- 2개 이상 프로젝트 선택 시, 표의 "단가" 칸이 프로젝트별로 각각 직접 입력 가능한 필드로 바뀜(`setRateForPjt`)
- 상단 "기술인력 단가" 입력은 신규 프로젝트 추가 시 기본값으로만 쓰이고, 개별 프로젝트에 단가를 따로 입력하면 그 값이 우선(`rateForPjt`)
- `⟳ 공수 불러오기`로 재조회해도 공수(`mandayOverride`)는 새로 받아온 값으로 갱신되지만, 단가 개별설정(`mandayRateOverride`)은 유지됨 — 공수만 다시 불러오고 싶을 때 단가를 매번 재입력할 필요 없음
- 항목표(`lines`)에도 프로젝트별 실제 적용 단가가 비고("공수 X × Y원")에 함께 기록됨
- 1개 프로젝트만 선택된 경우는 기존과 동일(공통 단가 입력 그대로 사용)

**상세**: `7_25_log_gihoek_settle_rate_per_pjt.md`

---

## 공수 연동(manday) 여러 프로젝트 합산 발행 — 1.4.x (r8, 2026-08-18)

**요청**: 지급예정서를 "공수 연동"으로 작성할 때, 지금까지는 프로젝트 1개만 정산 가능해서 Ph2·Ph4를 각각 별도 문서로 발행해야 했음. 하나의 정산서에 여러 프로젝트를 합쳐 발행할 수 있게 해달라는 요청.

**구현**:
- `settleDraft.pjtIds`(배열) 신규 — 상단 "프로젝트" 단일 선택과는 별개로, "공수 연동" 카드 안에 합산 대상 프로젝트를 다중 체크박스로 선택
- 1개 프로젝트만 선택된 경우 기존 UI/로직 그대로 유지(하위호환) — 총 공수 1줄 입력
- 2개 이상 선택 시 프로젝트별로 행이 나뉘어 표시되고(`mandayRowsHtml`), 각 행의 공수를 개별로 직접 수정 가능(`d.mandayOverride[pjtId]`). 단가는 공통(`mandayRate`)
- `⟳ 공수 불러오기`(`fetchManday`)는 선택된 모든 프로젝트를 순회하며 각자의 연동 컬렉션(`worker_manday`/`ph4_manday`)에서 근로자 명부 필터링을 거쳐 합산(`d.mandayBreakdown`)
- 항목(`lines`)도 프로젝트가 2개 이상이면 "2026-08 기술인력 투입 (P4 Ph2 FAB)" / "(P4 Ph4 SUP)"처럼 프로젝트별로 나뉘어 저장 — 인쇄물에서도 어느 프로젝트분인지 구분 가능
- 정산서 목록·상세·인쇄 화면의 프로젝트 배지도 다중 선택 시 "P4 Ph2 (FAB) + P4 Ph4 (SUP)"처럼 표시
- 목록 필터(PJT 필터)는 `pjtId` 또는 `pjtIds` 배열에 포함되면 매치되도록 확장

**적용 범위**: "공수 연동" 방식에만 적용. "공정 자동"·"수량 정산" 방식은 견적서가 프로젝트별로 종속되어 있어 이번 범위에서 제외(필요 시 별도 요청).

**상세**: `7_23_log_gihoek_settle_multi_pjt.md`

---

## 공수 연동(manday) 자동집계 오차 수정 — 1.4.x (r4, 2026-07-30)

**문제**: `fetchManday()`가 `worker_manday`/`ph4_manday` 컬렉션의 일별 문서에서 `md` 객체 값을 **무조건 전부 합산**하고 있어, 월간 공수 집계 화면(`pjt_manday`)의 PJT별 합계보다 큰 값이 표시되는 경우가 있었다. (예: SUP 7월 실제 8.0공수인데 정산서엔 19.6공수로 집계)

**원인**: `pjt_manday` 화면은 합산 시 해당 근로자 ID가 **현재 유효한 명부**(`pjt_workers_fab`/`pjt_workers_ph4`)에 있는 경우만 카운트하고, 명부에 없는 ID(근로자 정리·중복 통합·오입력 등으로 남은 잔재 데이터)는 무시한다. 반면 `gihoek`의 `fetchManday()`는 이런 필터링 없이 일별 문서의 `md` 값을 전부 더했기 때문에 두 화면의 집계 기준이 달라 수치가 어긋났다.

**수정**: `fetchManday()`도 동일하게 프로젝트에 연동된 명부 컬렉션(FAB→`pjt_workers_fab`, SUP→`pjt_workers_ph4`)을 조회해 ID셋을 만들고, 그 안에 있는 ID의 공수만 합산하도록 변경. 명부 외 ID의 공수는 제외되며, 있을 경우 완료 토스트에 "(명부 외 N 공수 제외)"로 안내.

---

## 정산서-견적 연결(perEst 수동 매핑) — 1.4.x (r5, 2026-07-30)

**배경**: 공정(progress) 정산서의 "기청구"는 `priorByEst(estId)`가 `docType==='invoice'`인 정산서 중 `perEst[estId]`가 있는 것만 합산한다. `perEst`는 발행 시 `method==='progress'`나 `'qty'`일 때만 자동 저장되므로, **직접입력(manual)**으로 발행된 과거 청구서는 실제로는 같은 견적/설비에 대한 청구였어도 `perEst`가 비어있어 이후 공정 정산서의 기청구에 반영되지 않는 문제가 있었다.

**해결**: 정산서 상세 화면에 **"🔗 견적 연결"** 버튼 추가 (`docType==='invoice'` && `method`가 `manual`/`qty`인 경우 노출).

- `linkSettleToEst(id)`: 같은 PJT의 유효 견적 목록에서 번호로 선택 → 이 정산서 금액 중 해당 견적에 기청구로 반영할 금액을 입력 → `gihoek_settlements/{id}`에 `{perEst:{[estId]:금액}}` merge 저장
- 연결되면 상세화면에 "🔗 견적 연결됨: [견적명] [금액]원" 안내 표시, 버튼 라벨도 "견적 연결 수정"으로 변경(재연결 가능)
- 연결 후에는 해당 견적으로 작성하는 **다음 공정 정산서부터** `priorByEst()`가 이 금액을 인식해 기청구에 자동 반영됨
- ⚠️ 소급 반영이므로, 과거에 직접입력으로 청구했던 물량/설비가 있다면 담당자가 수동으로 견적과 연결해줘야 함 (자동 매칭 없음)

---

## 정산서-견적 연결 — 항목(내역)별 개별 지정으로 개선 — 1.4.x (r6, 2026-07-30)

r5에서 도입한 "견적 연결"이 정산서 1건 전체에 대해 견적 1곳만 연결 가능했던 것을, **정산서의 내역(품목) 각각을 서로 다른 견적에 개별 연결**할 수 있도록 개선.

- `linkSettleToEst(id)`: 이제 prompt 대신 모달을 띄워 `s.lines`(청구서 내역) 각 항목 옆에 견적 선택 드롭다운을 표시
- 저장(`saveLinkEst`) 시 `gihoek_settlements/{id}`에 두 필드 기록:
  - `perEstLines`: `{라인인덱스: 견적ID}` — 모달 재오픈 시 이전 선택 상태 복원용
  - `perEst`: `{견적ID: 합산금액}` — 같은 견적에 연결된 여러 라인의 금액을 합산 (기존 `priorByEst()` 로직과 100% 호환, 별도 수정 불필요)
- 견적과 무관한 항목(예: "관리자 파견비")은 "— 연결 안 함 —"으로 두면 기청구 계산에서 제외됨
- 예시: "대금청구서 5개 내역(FIZ/X-OB/복합4동/복합3동/관리자파견) → FIZ 내역만 FIZ 견적에, 복합4동 내역만 복합4동 견적에" 각각 연결 가능

---

## 정산 목록 필터 (구분/PJT/거래처/상태) — 1.4.x (r7, 2026-07-30)

정산 목록(`renderSettle`) 상단에 필터 바 추가:

- **구분**: 전체 / 대금청구서 / 지급예정서 (`docType`)
- **PJT**: 전체 / 각 프로젝트 (`pjtId`)
- **거래처**: 전체 / `settlements`에 등장한 `recipient.company` 값에서 자동 추출한 목록
- **상태**: 전체 / ✓ 완결(`status==='done'`) / 미결(그 외)

필터 상태는 `window.settleFilter`(세션 메모리, 새로고침 시 초기화)에 저장되며, `setSettleFilter(key,val)`로 갱신 시 목록을 재렌더링. 상단 요약 카드(대금청구 누계/지급예정 누계/미수금)도 **필터가 적용된 결과 기준**으로 재계산됨. 필터가 하나라도 걸려있으면 "필터 초기화" 버튼이 노출됨.


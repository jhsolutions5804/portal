...
### 4. 배포 후 필수 확인 (무조건)
- 코드가 portal에 적용된 후, **예외 없이 아래 두 가지를 사용자에게 질문**한다.
  1. **백업 여부**: "현재 배포된 코드를 백업하시겠습니까?"
  2. **버전 번호**: "이 버전의 ver. no.를 지정해 주시겠습니까?"
- 사용자가 응답하기 전까지 다음 작업을 진행하지 않는다.
- 백업 지시 시 → `/home/claude/rollback/`에 캐시 저장 후 보고.
- ver. no. 지정 시 → 개발 로그(7.x.md)에 해당 버전 정보 기록.

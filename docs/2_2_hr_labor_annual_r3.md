...
> ⚠️ **2026-09-24 버그 수정(hr v2.12.1)**: 계약서 상세화면의 "‹ 목록" 버튼이 실제로는 아무도 진입하지 않는 옛 조회 화면(`renderLaborList2`)으로 돌아가던 버그를 수정 — 아래 흐름도대로 `renderLaborMain()`으로 정상 복귀. 연봉계약서(`2_3_hr_annual_r2.md`)도 동일 버그·동일 수정.
> 🆕 **2026-09-25 개정(hr v2.13.0)**: 목록에 🟢재직자/🔴퇴사자 필터 탭 신설(`laborSetFilter`) — 근로자 명부(2.1)와 동일한 UX. 기본은 재직자만 표시, 퇴사자 탭에서 과거 근로계약서도 조회 가능. `getWorkersList()`는 원래도 퇴사자 포함 전체를 반환했으나 구분 없이 한 목록에 섞여 있던 것을 분리.

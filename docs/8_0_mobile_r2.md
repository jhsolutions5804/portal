...
- 일정 카드 클릭 → 상세 화면.
- **(v2.1.0) 등록/완료/수정**: 우측 하단 `+` FAB → 모달 폼(내용·날짜·시간·태그·메모). 완료 체크는 문서 `done` 필드(다중일 일관). 상세에 수정 버튼. import에 addDoc/deleteDoc 추가.
...
- 공수: `worker_manday/{날짜}`(FAB)/`ph4_manday/{날짜}`(SUP), `{md:{wid:값}}`. **오늘 날짜 기술인별 입력칸(0.25단위, `window.setMd`)** (v2.1.0).
- **(v2.1.0) 일자 기준 06시**: `todayStr`에 `if(getHours()<6) 전날` 적용 → PC와 통일. `m/edoc`도 `bizToday()` 사용. (`toISOString()`=UTC 사용 금지)
...
## v2.1.0 변경 요약 (2026-07-06)

모바일 5대 과제 완성 + 로그인 게이트.

- **전자결재(`m/edoc.html`)**: 결재(승인/반려·관리자 대행)·작성 3종(업무일지·연차·초과근로)·삭제. 승인완료 문서 버튼 숨김, 퇴직원서 타일 배치. → `8_1_mobile_edoc`
- **PJT(`m/pjt.html`)**: 일정 등록/완료/수정, 근태 공수 입력, 06시 기준.
- **인사(`m/hr.html`)**: 모든 메뉴 근로자 명단 우선 → 클릭 시 해당 문서. 근로자명부·퇴직금 초과근로 미표시.
- **로그인 게이트(`m/auth-guard.js`·`index.html`)**: 로그인 정보 없이 서브페이지 직접 진입 차단→메인 유도·복귀, 무한루프 방지, splash fallback. → `0_3_portal_auth`

상세 로그: `7_8_log_2026-07-06_session.md` · 백업: `backup/v2.1.0/`

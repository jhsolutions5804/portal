...
> 최초 작성: 2026-07-01 · 최종 개정: 2026-09-25 (r7 · 모바일을 PC와 완전 동일화 — 업무일지 프로젝트연동/캘린더, 연차 11종+결재1직접지정+캘린더+전직원달력. 상세는 `3_6_mobile_edoc_parity_2026-09-25.md` §3·§4) · 2026-09-24 (r6 · 업무일지 캘린더 뷰 신설, 상세화면 복귀처 수정) · 2026-09-16 (r5 · 연차신청서 결재라인(approvers) 추가, 관리자 결재함 전체조회) · 2026-09-16 (r4 · 연차신청서 관리자 전 직원 신청 내역 조회 기능 추가) · 2026-09-16 (r3 · 결재 처리 후 결재요청(결재함)으로 복귀) · 2026-08-29 (r2 · 업무일지 권한오류 수정, 연차신청서 전면 개편) -20 (r1 · 업무일지 임시저장/반려 상태 수정 기능) · 작성: 춘식이(Claude)
...
## 업무일지 — 캘린더 뷰 (r6, 2026-09-24)

기존엔 업무일지가 목록(리스트) 형태로만 보였는데, 날짜 감각으로 보기 쉽도록 캘린더 뷰를 추가했다. `renderDailyMain()`이 `_dailyView`(`'list'`|`'calendar'`) 값에 따라 `renderDailyList()` 또는 `renderDailyCalendar()`로 분기.

- 월별 네비게이션(`dailyCalNav(delta)`) + "오늘" 바로가기(`dailyCalToday()`).
- 해당 월 문서를 한 번에 조회해 `_dailyCalDocs`에 캐싱 후 날짜별로 그리드에 배치(`renderDailyCalendarBody`).
- 날짜 클릭(`dailyCalPickDate`) 시 그 날 작성된 업무일지 목록 표시, 문서 클릭(`dailyCalOpenDetail`)으로 상세 진입.
- **상세화면 "‹ 목록" 복귀처 수정**: 캘린더 뷰에서 들어간 경우 목록이 아니라 **캘린더로** 돌아가야 하는데 예전엔 무조건 전체목록으로 갔음 — 진입 경로(`_dailyView`)를 기억해뒀다가 그대로 복귀하도록 수정.

## 관련 포털 탭 동기화

포털 하단 작업탭 라벨이 이 앱 내부 이동(예: 상세화면의 "🏠 전자결재 홈" 버튼)에도 정확히 따라가도록 `goTab()`이 `portal-embed` 모드에서 `postMessage`로 포털에 현재 화면을 알린다 — 상세는 `0_5_portal_tabs.md` 참고.

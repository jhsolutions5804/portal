...
> 최초 작성: 2026-06-26 · 최종 수정: 2026-07-06(r3) · 작성: 춘식이(Claude)
...
## 모바일 서브페이지 로그인 게이트 (2026-07-06, v2.1.0)

**배경**: 모바일 서브페이지(`m/*.html`)는 로그인 게이트 없이 Firestore만 초기화 → 브라우저 IndexedDB(`firebaseLocalStorageDb`)에 남은 Firebase 세션으로 데이터가 자동으로 보인다("자동 로그인"). 그러나 앱 로그인 정보(`jh_login_perms`: name·dept·rank·uid·admin)는 메인 `index.html` 로그인 시에만 저장되므로, 서브페이지 **직접 진입** 시 `_me`가 비어 승인/반려 등 권한이 잡히지 않는다.

> ⚠️ 테섭(`github.io/portal-test`)·운영(`github.io/portal`)은 **같은 origin**이라 IndexedDB/localStorage(및 Firebase 세션)를 공유한다. 로그아웃 시 세션이 꼬여 로딩에 갇힐 수 있다.

### `m/auth-guard.js` (모바일 공통)
- **게이트**: `jh_login_perms` 없이 진입 시 `../index.html`로 유도. 목적지를 `sessionStorage.jh_return_to`에 저장.
- **무한루프 방지**: `jh_gate_redir` 카운터(2회 초과 시 통과시켜 최소 화면은 뜨게). 정상 로그인 시 리셋.

### `index.html`
- `showApp()`: **복귀 경로(`jh_return_to`)가 있으면 UA와 무관하게** 원래 모바일 페이지로 복귀(PC UA에서 PC 홈으로 튕기던 문제 해결). 없으면 기존 UA 분기.
- **splash fallback**: 인증 콜백이 6초 내 미호출 시 로딩 강제 해제 + 로그인 화면 표시(로그아웃 후 로딩 갇힘 대응).

...
> 최초 작성: 2026-06-27 · 최종 수정: 2026-09-19(양식 동결 참조) · 최종 개정: 2026-09-24(목록 복귀 버그 수정) · 작성: 춘식이(Claude)
> ℹ️ **저장 서류 양식 동결(2026-09-19, hr v2.4.1)** — 저장된 서류는 저장 당시 양식(`tplVer`)으로만 조회·PDF 출력. 상세 `2_9_hr_saved_doc_freeze.md`
> ⚠️ **2026-09-24 버그 수정(hr v2.12.1)**: 계약서 상세화면(`annualViewWorker2`)의 "‹ 목록" 버튼이 실제로는 아무도 진입하지 않는 옛 조회 화면(`renderAnnualList2`)으로 돌아가던 버그 수정 — `renderAnnualMain()`(실제 진입 경로인 메인 목록)으로 정상 복귀하도록 변경. 근로계약서(`2_2_hr_labor_annual_r3.md`)도 동일 버그·동일 수정.
> 🆕 **2026-09-25 개정(hr v2.13.0)**: 목록에 🟢재직자/🔴퇴사자 필터 탭 신설(`annualSetFilter`) — 근로계약서(2.2)와 동일한 UX.
...
salary     = (basic + fixedNight + weekly) × 12
```
...
weekly   = hourly × 8 × 4.345
salary   = (basic + fixedOt + weekly) × 12
```
...
`max-width: 1100px` — 근로계약서 탭과 동일한 레이아웃.

**컬럼**: 사원번호 · 이름 · 소속 · 직급 · 구분 · 최신 계약일 · 계약 수 · 상태 · 조회

| 컬럼 | 설명 |
|------|------|
| 사원번호 | empNo — **사원번호 오름차순 정렬** |
| 이름 | 클릭 시 계약서 상세로 이동 |
| 소속 | dept |
| 직급 | rank |
| 구분 | 현장(노랑) / 사무(파랑) 배지 |
...
**상태 컬러**
- 유효: `#16A34A` 초록 (endDate ≥ 오늘)
- 만료: `#DC2626` 빨강 (endDate < 오늘)
- 미작성/없음: `#9AAABF` 회색
...
```
renderAnnualMain()  ← 탭 진입, PC 테이블 (사원번호 순)
  ├─ 이름 클릭 or 조회 버튼 → annualViewWorker2(wid, wname)
  │     ├─ 계약서 카드 목록 (계약일·기간·연봉·PDF출력·삭제)
  │     ├─ ← 목록 버튼 → renderAnnualList2() → renderAnnualMain()
  │     └─ 삭제 → annualDeleteContract2(wid, cid, wname)
  └─ ➕ 신규 작성 버튼 → renderAnnualRegister()
        ├─ annualWorkerSelect2(wid) : 근로자 선택 시 자동 채움
        ├─ annualUpdateSalaryPreview() : 시급 입력 시 연봉 실시간 계산
        ├─ annualPreview2() : A4 미리보기
        ├─ annualSave2() : Firestore 저장 후 renderAnnualMain()
        └─ ← 뒤로가기 → renderAnnualMain()
```

> `renderAnnualList2()`는 `renderAnnualMain()`의 래퍼로, 기존 참조 유지를 위해 존재.
...
| 함수 | 역할 |
|------|------|
| `renderAnnualMain()` | PC 테이블 목록 (탭 진입점), 사원번호 순 정렬 |
| `renderAnnualList2()` | renderAnnualMain() 래퍼 (기존 참조 호환) |

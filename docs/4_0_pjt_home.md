...
- **적용 범위**: PC 홈 캘린더는 PC 전용(모바일은 `m/home.html` 리다이렉트). 모달 z-index 40(기존 프로필 모달과 동일). **모바일은 `m/schedule.html`(통합 일정)에서 동일 스키마로 등록·수정·삭제·완료체크 지원(2026-09-19, 일정 1.1.0 → `8_0_mobile_r2.md`)**
- **완료체크 + 할 일 토글 (4.5.1, 2026-09-10 추가)**: 그동안 `isTodo` 필드는 스키마에만 존재하고 UI에 노출되지 않았음 → 등록/수정 폼에 "할 일(시간 없이 체크리스트로 등록)" 토글(`_hscSetTodo`) 추가, 켜면 시작/종료 시간 입력이 비활성화되고 저장 시 `isTodo:true`·`stime`/`etime` 공란 처리. 우측 상세 패널의 일정 목록에도 체크박스 추가(`toggleHomeSchedDone`) → 클릭 시 문서 `done`/`doneUpdatedAt` 필드로 즉시 저장(PJT 앱 `_usSetDone`과 동일한 패턴), 완료 시 취소선 표시. `isTodo` 항목은 목록에 "할일" 배지 표시

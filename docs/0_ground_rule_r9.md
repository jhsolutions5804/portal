...
> 최초 작성: 2026-06-26 · 최종 수정: 2026-07-01(r9) · 작성: 춘식이(Claude)
...
9. 코딩 완료 + md 파일 rev. 업로드 완료 후, **portal 전체를 통합 스냅샷으로 백업**한다.
   ...
   → **버전 번호는 Semantic Versioning(SemVer)을 따른다** (하단 'Semantic Versioning' 섹션 참조).
...
## Semantic Versioning (코드 버전 부여 규칙)

**code versioning은 Semantic Versioning(SemVer)을 따른다.**

버전 형식: **`vMAJOR.MINOR.PATCH`** (예: `v1.1.0`)

| 자리 | 올리는 경우 | 예시 |
|------|------------|------|
| **MAJOR** | 하위 호환이 깨지는 변경 (기존 기능·데이터 구조 비호환 변경) | v1.x.x → v2.0.0 |
| **MINOR** | 하위 호환되는 **기능 추가** | v1.0.x → v1.1.0 |
| **PATCH** | 하위 호환되는 **버그 수정·소규모 보정** | v1.0.5 → v1.0.6 |

### 적용 원칙
- 상위 자리를 올리면 하위 자리는 **0으로 초기화** (1.0.5 → 1.1.0, 1.4.2 → 2.0.0).
- 단순 오타·디버그·문구 수정은 PATCH.
- 새 메뉴·새 자동연동·새 입력기능 등은 MINOR.
- 기존 데이터 스키마를 못 읽게 만드는 변경은 MAJOR.
- 한 배포에 PATCH·MINOR 변경이 섞이면 **더 높은 등급**을 적용 (기능 추가 + 버그픽스 동시 → MINOR).
- 버전 등급 판단은 **춘식이가 먼저 제안**하고, 최종 번호는 사용자 확인 후 확정.

### 이력 예시
| 버전 | 등급 | 내용 |
|------|------|------|
| v1.0.5 | — | 6/29 버그픽스·정리 (J/K/L/M 코드) |
| v1.1.0 | MINOR | N2~N11 — 전자결재 PC개편, 용역비·급여 자동연동, 캐시방지 |

---

## 업데이트 절차 (2026-06-29 확정)

```
[요청] → 1.Logic검토 → (문제) → 2.대안제시
                ↓ (OK)
         3.최신코드 내려받아 Local Test
                ↓ (이상) → 4.Logic재검토 → 3번
                ↓ (OK)
         5.Test Server 배포
                ↓ (문제) → 6.Local 수정 → 5번
                ↓ (OK)
         7.Main Server 배포
                ↓ (문제) → 8.Test Server로 → 6번
                ↓ (OK)
         9.Main+Test 최신화 + MD rev.
```
...
| 구분 | URL | 저장소 |
|------|-----|--------|
| **main** | `jhsolutions5804.github.io/portal/` | `jhsolutions5804/portal` |
| **test** | `jhsolutions5804.github.io/portal-test/` | `jhsolutions5804/portal-test` |

- test server: 상단 빨간 배너 `⚠️ TEST SERVER` 표시
- Firebase DB: main/test 동일 프로젝트 (`p4ph2-fab-506a7`) 사용
- test server 배포 없이 main 직접 배포는 **긴급 hotfix + 사용자 명시 승인** 시만 허용

## 테스트 방식 정의

### 1단계 — Unit Test
- 개별 함수·메서드 단위 독립 검증 (`node` 스크립트)

### 2단계 — Integration Test
- 모듈 결합 후 interaction·데이터 흐름 검증

### 3단계 — Acceptance Test
- 실제 코드 파일에 수정 적용 후 before/after 검증
- `node --check` JS 문법 검증 필수

### 4단계 — 배포
- 3단계 모두 통과 → test server 배포 → main 배포

...
기존 `generateProgressPPT`(1P 갑지 + 2P 현황)에 **3~8P 섹터별 상세** 추가.

- 섹터당 1P — 4단계(반입/입고검사/설치/시공검측) × [전일·금일·누계·증감·공정율·진행바]
- 데이터: 함수가 이미 계산한 `todayStats`(누계)·`prevStats`(전일) 재사용. 금일=누계−전일, 증감=+금일. 추가 쿼리 없음
- 활동 없는 섹터는 `0` + 진행바 `-`
- 삽입 지점 1곳(`// 다운로드` 직전)만 추가 → 1·2P 로직 무변경. 테섭(`8a416eb2`)→운영(`59d814f1`) 반영, `node --check` 통과

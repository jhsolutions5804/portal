...
> 최초 작성: 2026-06-26 · 최종 수정: 2026-09-16(r7 · 중복 렌더함수 통합 버그 수정 + 재직자/퇴사자 필터 탭) · (r6 · 퇴직 처리/재직 복구 기능 추가) · 2026-06-29(r5) · 작성: 춘식이(Claude)
...
## 중복 렌더 함수 버그 수정 + 재직자/퇴사자 필터 탭 (r7, 2026-09-16)

**문제 1 (버그)**: `renderRosterMain`이 파일 내에 **두 번 정의**되어 있어(뒤에 정의된 것이 최종 적용), 근로자 명부 탭 진입 시에는 정상 테이블(사원번호/이름/소속/직급/구분/생년월일/휴대폰/근로계약/연봉계약/포털 10열 + 신규등록 버튼)이 보였지만, `rosterDetailSave`/`rosterDelete`/`rosterEmpNoSave`/`rosterLinkPortal`/`rosterResign`/`rosterUnresign` 등 **모든 수정·저장 액션은 완전히 별도로 존재하던 구버전 함수 `renderRosterList`**(6열짜리 카드형 목록, 신규등록 버튼 없음, 뒤로가기 버튼)를 호출해 화면을 갱신하고 있었음 — "수정하면 화면이 이상하게 바뀐다"는 현상의 원인.

**수정**: 죽은 코드(항상 덮어써지던 첫 번째 `renderRosterMain` 정의)와 더 이상 쓰이지 않게 된 구버전 `renderRosterList` 함수를 완전히 삭제. 모든 저장/삭제/퇴직처리 액션의 갱신 호출을 `renderRosterMain()` 하나로 통일 — 이제 어떤 조작을 하든 항상 동일한 정상 테이블 화면으로 돌아옴.

**문제 2 (요청)**: 대표님 — "근로자 명부에서 퇴사자 따로 볼 수 있게 해주고."

**수정**: `renderRosterMain` 상단에 **🟢 재직자 (N) / 🔴 퇴사자 (N)** 필터 탭 추가(`rosterSetFilter('active'|'resigned')`, 모듈 전역 `_rosterViewFilter`). 기본값은 재직자 탭 — 목록이 퇴사자로 지저분해지지 않음. 퇴사자 탭에서는 이름 옆에 `퇴직 YYYY-MM-DD` 배지 표시. 상세 모달의 퇴직/복구 버튼은 기존 그대로(r6) 동작.

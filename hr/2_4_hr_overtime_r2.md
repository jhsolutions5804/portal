...
> `reason`(사유)은 2026-07-21 추가된 필드입니다. 선택 입력이며, 값이 없으면 화면에 "-"로 표시됩니다. 기존 데이터는 마이그레이션 없이 그대로 호환됩니다.
...
## 전자결재(edoc) 연동 — 초과근로 상신

- `edoc/index.html`에 별도의 **초과근로 상신** 화면 존재 (결재 라인: 작성 → 결재 김종화 차장 → 회람 김민서 대리)
- 상신 문서는 `edoc_overtime` 컬렉션에 저장되며, **결재 승인 시** 이 문서의 `hr/overtime` 컬렉션에 자동으로 문서가 추가됨 (`linkedToOvertime` 플래그로 중복 등록 방지)
- 사유(`reason`) 필드는 상신 폼에서 입력 → 승인 시 `hr/overtime` 컬렉션에도 함께 복사됨
- 관련 문서: `edoc/3_x_edoc_*.md` (전자결재 모듈 문서 참조)
